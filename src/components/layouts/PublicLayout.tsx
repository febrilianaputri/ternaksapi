...
import { FaBars, FaTimes, FaMoon, FaSun, FaChevronRight, FaCamera } from "react-icons/fa";
import { useTheme } from "@/context/ThemeContext";
import { useAuth } from "@/context/AuthContext";
import { TCowLogo } from "@/components/ui/TCowLogo";
import { ScanOverlay } from "@/components/layouts/ScanOverlay";
        ...
        <div className="max-w-9xl mx-auto px-6 lg:px-12">
          <div className="flex items-center justify-between h-16">
            ...
            <div className="hidden md:flex items-center gap-1 rounded-full border border-brand-forest/10 dark:border-brand-accent/15 bg-brand-cream/80 dark:bg-brand-forest/25 px-1.5 py-1.5 shadow-sm">
              {navLinks.map((link, i) => (
                <Link key={`${link.href}-${i}`} href={link.href} className={`rounded-full px-4 py-1.5 text-sm transition-colors ${isActive(link.href) ? linkActive : linkBase}`}>
                  {link.label}
                </Link>
              ))}
            </div>
